...
var AuthenticationContext = (function () {
    ...
    */
     ...
     * @ignore
     */
    AuthenticationContext.prototype._renewIdToken = function (callback, getBothIdAccessToken, _resource) {
        // use iframe to try refresh token
        ...
        var resource = null;
        if (getBothIdAccessToken === true) {
            responseType += '+token';
            resource = _resource;
        }
        var urlNavigate = this._getNavigateUrl(responseType, resource) + '&prompt=none';
        urlNavigate = this._addHintParameters(urlNavigate);
     ...
     * @param {tokenCallback} callback -  The callback provided by the caller. It will be called with token or error.
     */
    AuthenticationContext.prototype.acquireToken = function (resource, getBothIdAccessToken, callback) {
        ...
        if (this._activeRenewals[resource]) {
            ...
            if (getBothIdAccessToken === true) {
                this.verbose('renewing idtoken & access token');
                this._renewIdToken(callback, getBothIdAccessToken, resource);
                ...
                this.verbose('renewing access token');
