...
var AuthenticationContext = (function () {
    ...
    AuthenticationContext = function (config) {
        ...
        this._renewStates = [];
        this._checkSessionEndpoint = 'https://login.microsoftonline.com/common/oauth2/checksession';
        this._checkSessionIframe = null;
        this._sessionState = null;
        this._acquireTokenCalls = [];
        ...
        if (!this.config.redirectUri) {
            ...
            this.config.redirectUri = this.config.redirectUri.replace(hash, '');
        }
    ...
    */
    ...
    AuthenticationContext.prototype.getUserAsync = function (callback) {
        this._checkSession();
        var self = this;
        return new Promise(function (resolve, reject) {
            window.addEventListener('adal:userSessionChanged', function (e) {
                if (e.detail == "unchanged") {
                    var idtoken = self._getItem(self.CONSTANTS.STORAGE.IDTOKEN);
                    if (!self._isEmpty(idtoken)) {
                        self.info('User exists in cache: ');
                        self._user = self._createUser(idtoken);
                    }
                    resolve(self._user);
                }
                else {
    ...
    /**
    * Sends a post message to check session endpoint using a hidden iframe.
    * @ignore
    */
    AuthenticationContext.prototype._checkSession = function () {
        console.log("checkSession");
        try {
            var frameId = "adalSession";
            this._checkSessionIframe = this._addAdalFrame("adalSession");
            this._checkSessionIframe.src = this._checkSessionEndpoint;
            this._boundMessageEvent = this._receiveMessage.bind(this);
            if (window.addEventListener) {  // For all major browsers, except IE 8 and earlier
                window.addEventListener("message", this._boundMessageEvent, false);
            } else if (window.attachEvent) {                  // For IE 8 and earlier versions
                window.attachEvent("onmessage", this._boundMessageEvent);
            }
            var self = this;
            setTimeout(function () {
                var sessionState = self._getItem(self.CONSTANTS.STORAGE.SESSION_STATE) || self._guid();
                var expectedState = self.config.clientId + ' ' + sessionState;
                self._checkSessionIframe.contentWindow.postMessage(expectedState, self.instance.replace(/\/$/, ""));
            }, 300);
        } catch (err) {
            this.error('Error navigating iframe to checkSession endpoint', err);
        }
    }

    /**
    * Event Handler to receive message from the check session endpoint with the sessionstate received as changed/unchanged/error.
    * @ignore
    */
    AuthenticationContext.prototype._receiveMessage = function (e) {
        if (e.origin === this.instance.replace(/\/$/, "") && e.source === this._checkSessionIframe.contentWindow) {
            if (e.data === "error") {
                this.info("error message from check session op iframe");
                this._sessionState = e.data;
            }
            else if (e.data === "changed") {
                this.info("Session state: " + e.data + " received from check session op iframe");
                this._sessionState = e.data;
                this._userSessionChanged(e.data);
            }
            else {
                this.info("Session state: " + e.data + " received from check session op iframe");
                this._sessionState = e.data;
    ...
    /**
    * Custom event to alert the user Session   .
    * @ignore
    */
    AuthenticationContext.prototype._userSessionChanged = function (user) {
        var evt = new CustomEvent('adal:userSessionChanged', { detail: user });
        window.dispatchEvent(evt);
    };
