        ...
        AdalModule.provider('adalAuthenticationService', function () {
            ...
            this.$get = ['$rootScope', '$window', '$q', '$location', '$timeout', '$injector', function ($rootScope, $window, $q, $location, $timeout, $injector) {
                ...
                return {
                    ...
                    userInfo: _oauthData,
                    acquireToken: function (resource, getBothIdAccessToken) {
                        // automated token request call
                        var deferred = $q.defer();
                        _adal._renewActive = true;
                        _adal.acquireToken(resource, getBothIdAccessToken, function (errorDesc, tokenOut, error) {
                    ...
                    if (config) {
                        ...
                        if (tokenStored) {
                            ...
                            if (authService.loginInProgress()) {
                                ...
                                var delayedRequest = $q.defer();
                                var getBothIdAccessToken = false;

                                if (
                                    typeof config.bothIdAccessToken !== 'undefined' &&
                                    config.bothIdAccessToken === true
                                ) {
                                    getBothIdAccessToken = true;
                                }

                                authService.acquireToken(resource, getBothIdAccessToken).then(function (token) {
